...
export function useInstallSkill() {
  ...
  return useMutation({
    mutationFn: ({ projectId, skillId, systemIds }: { projectId: string; skillId: string; systemIds: string[] }) =>
...
export function useUnlinkSystem() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: skillKeys.projectSystems(variables.projectId) });
      queryClient.invalidateQueries({ queryKey: skillKeys.byProject(variables.projectId) });
    },
  });
}
