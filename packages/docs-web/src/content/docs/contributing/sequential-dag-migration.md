---
title: Sequential-to-DAG Migration
description: Guide for migrating legacy sequential workflow definitions to the current DAG node format.
...
:::caution[Deprecated]
This guide is historical. All workflows now use the DAG `nodes:` format exclusively. The sequential `steps:` format has been removed. This page is preserved for users who may still need to migrate old workflow files.
:::
