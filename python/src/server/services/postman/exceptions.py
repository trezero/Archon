        ...
        if not message:
            ...
            if original_error:
                ...
                if "NameResolutionError" in error_str or "Failed to resolve" in error_str or "Temporary failure in name resolution" in error_str:
                    message += (
                        "\n\nDNS RESOLUTION ERROR - NETWORK RESTRICTION DETECTED"
                        "\n\nThis may be caused by network security restrictions."
                        "\n\n'api.getpostman.com' may not be in the network allowlist."
                        "\n\nSOLUTIONS:"
                        "\n1. Check your network/firewall settings"
                        "\n2. Verify DNS resolution for api.getpostman.com"
                ...
                if "ProxyError" in error_str or "Tunnel connection failed" in error_str or "403 Forbidden" in error_str:
                    message += (
                        ...
                        "\n\nSOLUTION: The client bypasses proxies by default."
                        "\n\nIf you're still seeing this error:"
                        "\n1. Ensure use_proxy is not set to True in your config"
                        "\n2. If in a corporate environment, configure your proxy to allow api.getpostman.com"
                    )
                    return super().__init__(message)
